...
if(!$user->isLoggedIn()) { // check user login (user should not be logged in)
    ...
    if($user->getData()->role != 'optician' && $user->getData()->role != 'patient') {
        ...
        if(!$db->error()){
            Session::flash('successMessage', '<br><br><div class="alert alert-success"><strong>Success!</strong><br>You have removed an appointment from the list.</div>');
        }else{
